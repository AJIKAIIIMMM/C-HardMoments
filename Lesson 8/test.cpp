...
class PhoneBook : public Person, PhoneNumber {
private:
...
public:
    ...
    friend std::ostream& operator << (std::ostream& os, const PhoneBook&);
...
std::ostream& operator << (std::ostream& os, const PhoneBook& phb) {
...
TEST_F(TestPhoneBook, methods) {
    book->SortByName();
    ASSERT_EQ(book->m_data[0], pair<Person, PhoneNumber>);
    book->SortByPhone();
    ASSERT_EQ(book->m_data[0], pair<Person, PhoneNumber>);
    book->GetPhoneNumber("Sokolov");
    ASSERT_EQ(book->m_data[0], pair<Person, PhoneNumber>);
}
